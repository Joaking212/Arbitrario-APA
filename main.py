...
def instalar_mod():
    ruta_destino = filedialog.askdirectory(title="Seleccione una carpeta de destino")
    if ruta_destino:
        try:
            # Ruta de la carpeta "Mods" que quieres copiar
            ruta_mods = "./files_install/Mods"
            # Copiar la carpeta "Mods" a la ruta de destino seleccionada
            shutil.copytree(ruta_mods, ruta_destino + "/Mods")

            # Rutas de los archivos que quieres copiar
            ruta_archivo1 = "./files_install/UE4SS-settings.ini"
            ruta_archivo2 = "./files_install/xinput1_3.dll"
            # Copiar los archivos a la ruta de destino seleccionada
            shutil.copy(ruta_archivo1, ruta_destino)
            shutil.copy(ruta_archivo2, ruta_destino)

            messagebox.showinfo("Éxito", "El MOD se ha instalado correctamente.")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo instalar el MOD: {e}")
...
instalar_mod_button = tk.Button(archivo_frame, text="Instalar MOD", command=instalar_mod)
instalar_mod_button.grid(row=0, column=3, padx=5, pady=5)
